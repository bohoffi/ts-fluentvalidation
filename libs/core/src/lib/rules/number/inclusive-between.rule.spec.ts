...
describe(InclusiveBetweenRule.name, () => {
  ...
  it('should return an error if the value is outside the bounds', () => {
    ...
    result.shouldHaveValidationErrorFor(p => p.age).withMessage('age must be between 18 and 65.');
  });

  it('should fall back to `0` if the value is nullish', () => {
    const fallbackValidator = createValidator<Person>(val => val.ruleFor('age').inclusiveBetween(-10, -5));
    const result = testValidate(fallbackValidator, createPersonWith({ age: null as unknown as number }));
    expect(result.isValid).toBeFalsy();
    expect(result.errors).toHaveLength(1);
    result.shouldHaveValidationErrorFor(p => p.age).withMessage('age must be between -10 and -5.');
  });
});
