...
describe(LessThanOrEqualRule.name, () => {
  ...
  it('should return an error if the value is greater than the reference value', () => {
    ...
    result.shouldHaveValidationErrorFor(p => p.age).withMessage('age must be less than or equal to 10.');
  });
  ...
  it('should fall back to `0` if the value is nullish', () => {
    ...
    expect(result.isValid).toBeFalsy();
    expect(result.errors).toHaveLength(1);
    result.shouldHaveValidationErrorFor(p => p.age).withMessage('age must be less than or equal to -1.');
  });
});
