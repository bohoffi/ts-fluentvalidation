 ...
 */
function mergeValidations<TModel extends object, Key extends KeyOf<TModel>, Validations extends object>(
  validator: Validator<TModel, Validations>,
  key: Key,
  applyConditions: boolean,
  ...validations: Validation<TModel[Key], TModel>[]
): Validator<TModel, Validations & { [P in Key]: Validation<TModel[Key], TModel>[] }> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const existingValidations = ((validator.validations as any)[key] ?? []) as unknown as Validation<TModel[Key], TModel>[];
  ...
  return validator as Validator<TModel, Validations & { [P in Key]: Validation<TModel[Key], TModel>[] }>;
}
